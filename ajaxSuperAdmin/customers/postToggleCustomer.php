...
if(!isset($ret['error'])){
    ...
    $value = $dbh->query("SELECT ".$post['field']." FROM customers WHERE id = ".$post['id'])->fetch(PDO::FETCH_COLUMN);
    $sth = $dbh->prepare("UPDATE customers SET ".$post['field']." = ".($value?'null':1)." WHERE id = ".$post['id']);
    if( $sth->execute() ){
